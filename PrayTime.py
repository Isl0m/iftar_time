...
#-------------------------- Test Code --------------------------
def getTime():
	time = prayTimes.getTimes(datetime.date.today(), (41.311081, 69.240562),5)
	timeNow = dateCalc.now(pytz.timezone('Asia/Tashkent')) #- relativedelta(minutes=1)
	timeNow = timeNow.strftime("%H:%M")
	sunset = time['sunset']
	time_1 = dateCalc.strptime(timeNow,"%H:%M")
	time_2 = dateCalc.strptime(sunset,"%H:%M")
	return getDiference(time_1, time_2)